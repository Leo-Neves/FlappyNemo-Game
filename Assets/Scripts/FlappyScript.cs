...
public class FlappyScript : MonoBehaviour
{
    ...
    public float RotateUpSpeed = 0, RotateDownSpeed = 0;
    public GameObject IntroGUI, DeathGUI;
    public Collider2D restartButtonGameCollider;
    public float VelocityPerJump = 0.2F;
    ...
    void Update()
    {
        ...
        {
            MoveBirdOnXAxis();
            if (WasTouchedSpace() || WasTouchedMouse())
            {
        ...
        else if (GameStateManager.GameState == GameState.Playing)
        {
            ...
            if (WasTouchedUp())
                BoostOnYAxisUp();
            else if (WasTouchedDown())
                BoostOnYAxisDown();
            else if (SoltouTeclaPraCima())
                PararDeMover();
            else if (SoltouTeclaPraBaixo())
                PararDeMover();
        ...
        else if (GameStateManager.GameState == GameState.Dead)
        {
            ...
            if (restartButtonGameCollider == Physics2D.OverlapPoint
                (Camera.main.ScreenToWorldPoint(contactPoint)) && false)
            {
    ...
    bool WasTouchedSpace(){
        return Input.GetKeyDown(KeyCode.Space);
    }

    bool SoltouTeclaPraCima(){
